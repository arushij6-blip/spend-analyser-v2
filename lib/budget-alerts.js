...
export async function checkAndSendBudgetAlerts({ month } = {}) {
  ...
  const budgets = getBudgetsForMonth(mo);
  if (budgets.length === 0) {
    return { sent: [], skipped: 0, configured };
  }

  const spendByCat = getCategorySpendForMonth(mo);
  const sent = [];
  let skipped = 0;
  ...
  for (const { category, amount } of budgets) {
    ...
    for (const threshold of THRESHOLDS) {
      ...
      if (wasAlertSent(category, mo, threshold)) {
        skipped++;
        continue;
      }
      if (!configured) {
      ...
      if (ok) {
        markAlertSent(category, mo, threshold);
